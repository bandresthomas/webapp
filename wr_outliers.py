...
def create_wr_graph(df_filtered, tiers_range, title_suffix, filename_suffix):
    """
    Cria um gráfico para os WRs com os tiers especificados
    """
    ...
    # Adiciona título
    ax.set_title(f'Análise de Outliers - WRs Fantasy 2025 {title_suffix}\n(Tiers {min(tiers_range)}-{max(tiers_range)})', 
    ...
    # Salva o gráfico
    plt.savefig(f'images/wr_outliers_{filename_suffix}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Print das estatísticas
    print(f"\n=== ESTATÍSTICAS WRs TIERS {min(tiers_range)}-{max(tiers_range)} ===")
...
def create_wr_outliers_graphs():
    """
    Cria os gráficos de outliers dos WRs baseado no arquivo CSV
    """
    # Carrega os dados
    df = pd.read_csv('tables/FantasyPros_2025_Draft_WR_Rankings.csv')
    ...
    print(f"Total de WRs válidos encontrados: {len(df_filtered)}")
    ...
    print("="*50)
    create_wr_graph(df_filtered, range(1, 6), "(Tiers 1-5)", "tiers_1_5")
    
    # Cria o gráfico para tiers 6-10
    print("\n" + "="*50)
    print("CRIANDO GRÁFICO PARA TIERS 6-10")
    print("="*50)
    create_wr_graph(df_filtered, range(6, 11), "(Tiers 6-10)", "tiers_6_10")

if __name__ == "__main__":
    # Garante que a pasta images existe
    os.makedirs('images', exist_ok=True)
    
    create_wr_outliers_graphs()
