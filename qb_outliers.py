...
def create_qb_outliers_graph():
    """
    Cria o gráfico de outliers dos QBs baseado no arquivo CSV
    """
    # Carrega os dados
    df = pd.read_csv('tables/FantasyPros_2025_Draft_QB_Rankings.csv')
    
    # Filtra apenas os primeiros 30 jogadores ordenados por ranking
    df_filtered = df.head(30).copy()
    
    # Converte fantasypts para numérico, removendo aspas
    df_filtered['FANTASYPTS'] = pd.to_numeric(df_filtered['FANTASYPTS'].astype(str).str.replace('"', ''), errors='coerce')
    
    # Remove jogadores com fantasypts 0 ou NaN
    df_filtered = df_filtered[df_filtered['FANTASYPTS'] > 0].copy()
    ...
    tiers = df_filtered['TIERS'].unique()
    tier_positions = {tier: i for i, tier in enumerate(sorted(tiers))}
    ...
    # Configura o eixo X
    tier_labels = [f'Tier {tier}' for tier in sorted(tiers)]
    ...
    # Adiciona título
    ax.set_title('Análise de Outliers - QBs Fantasy 2025\n(Primeiros 30 Jogadores)', 
    ...
    # Print das estatísticas para nortear a visão gráfica
    print(f"\n=== ESTATÍSTICAS DOS PRIMEIROS 30 QBs ===")
    ...
    print(f"Tiers encontrados: {sorted(tiers)}")
